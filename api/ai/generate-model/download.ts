...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    return finalize(
      new Response(arrayBuffer, {
        ...
        headers: {
          ...
          'Content-Disposition': `attachment; filename="generated-${taskId}.glb"`,
        },
      })
    );
  } catch (error) {
    captureModelGenerationException(
      error,
      { taskId, stage: 'download' },
      'ai_generate_model_download_error'
    );
    return finalize(jsonResponse({ error: 'Failed to download generated model.' }, { status: 502 }));
  }
}
