...
export function ScenePanel({
  ...
  const handleUploadFile = useCallback(
    ...
    [isUploading, onUploadBackground]
  );
  ...
  const uploadCardClasses = useMemo(() => {
    ...
    if (isUploading) return `${base} cursor-not-allowed border-slate-300/70 opacity-80`;
          ...
          >
            <div className="flex flex-col items-center justify-center gap-2 text-center">
              ...
              <p className="text-sm font-semibold text-slate-800">
                {isUploading ? 'Uploading 360 image...' : 'Upload 360 Image'}
              </p>
              <p className="text-xs text-slate-500">Drag and drop or click to browse</p>
              <p className="text-[11px] text-slate-400">JPG, PNG, WebP (max 25MB)</p>
            </div>
          </button>
        ) : (
          <div className="rounded-[16px] border border-slate-200/80 bg-white/60 p-3">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-slate-500">{formatBytes(backgroundImage.fileSize)}</p>
              </div>
            </div>
